...
function App() {
  ...
  // EDITABLE QUOTATION DATE (YYYY-MM-DD)
  ...
  const loadExistingQuotations = async () => {
    ...
    try {
      const { data: quotationRows, error: quotationError } = await supabase
        .from("quotations")
        .select(
          "id, quotation_number, quotation_date, client_shop_name, client_name, client_phone, grand_total, created_at, user_id"
        )
  ...
  const openExistingQuotation = async (id) => {
    ...
    try {
      ...
      if (quotation.quotation_date) {
        ...
        setQuotationDate(new Date(quotation.created_at).toISOString().split("T")[0]);
      }
  ...
  const getQuotationPayload = () => {
    return {
      ...
      quotation_date: quotationDate, // 👈 SAVES DATE TO SUPABASE
  ...
  // =========================================================
  // SAVE EVERYTHING TO USER'S ACCOUNT IN SUPABASE
  // =========================================================
